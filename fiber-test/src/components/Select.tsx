...
import type {ListBoxItemProps, SelectProps as AriaSelectProps, ValidationResult, ListBoxProps} from 'react-aria-components';
  ...
  return (
    (
      <AriaSelect style={{margin: '-30px auto auto 0px'}} {...props}>
        {label && <Label style={{marginLeft: '-170px', fontSize: "1.5em"}}>{label}</Label>}
        <Button style={{display: 'flex', alignItems: 'center', width:'120px', height: '50px'}}>
          <SelectValue />
          <ChevronDown />
        </Button>
